...
get '/system_registry/services/configurations/tree' do
 ...
 get '/system_registry/services/configuration/' do
  @@system_registry.get_service_configuration(self.symbolize_keys(params)).to_json
 end
 
 post '/system_registry/services/configurations/' do
    if  @@system_registry.add_service_configuration(self.symbolize_keys(params)).to_json
      status(202)
    else
      status(404)
    end    
 ...
 if  @@system_registry.update_service_configuration(self.symbolize_keys(params)).to_json
   status(202)
 else
   status(404)
 end    
...
delete '/system_registry/services/configurations/' do
 if  @@system_registry.rm_service_configuration(self.symbolize_keys(params)).to_json
    status(202)
  else
    status(404)
  end    
