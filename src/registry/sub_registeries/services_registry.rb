...
#  end
  ...
  def service_is_registered?(service_hash)
    STDERR.puts('add_service_configuration ' + @registry.to_s + ' ' + service_hash.to_s  + ':' + [:parent_engine, :service_handle].to_s)
...
#    log_exception(e)
  ...
  def remove_from_services_registry(service_hash)
    ...
    raise EnginesException.new('Fail to find service for removal' + service_node.to_s, :error, service_node) unless service_node.is_a?(Tree::TreeNode)
  ...
  def find_service_consumers(service_query_hash)
     match_nstp_path_node_keys(@registry, service_query_hash, [], [:parent_engine, :service_handle])
...
#    if service_query_hash[:service_handle].nil? ||service_query_hash.key?(:service_handle) == false
 #     return services unless services.is_a?(Tree::TreeNode)
  #    return services if services.has_children?
