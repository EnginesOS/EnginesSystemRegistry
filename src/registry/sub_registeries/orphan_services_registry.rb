class OrphanServicesRegistry < SubRegistry
  ...
  def orphanate_service(params)
    raise EnginesException.new('Cannot Orphanate as Existing entry' +params.to_s)  if is_ns_tp_node_registered?(@registry, params, [:parent_engine, :service_handle]) == true
