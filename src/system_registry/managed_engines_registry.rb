class ManagedEnginesRegistry < SubRegistry
  def find_engine_services(params)
    ...
    return log_error_mesg('fail to find engine type tree', params) if !engines_type_tree.is_a?(Tree::TreeNode)
    engine_node = engines_type_tree[params[:parent_engine]]
   return log_error_mesg('fail to find engine in type tree', params)  if !engine_node.is_a?(Tree::TreeNode)
  ...
  def find_engine_services_hashes(params)
    ...
    return log_error_mesg('Failed to find engine type node' , params) if !engine_node.is_a?(Tree::TreeNode)
    ...
    if params.key?(:service_handle) && !params[:service_handle].nil?
      ...
      return order_hashes_in_priotity(get_all_leafs_service_hashes(engine_node)) if !params.key?(:persistant)
  ...
  def get_engine_persistance_services(params, persistance) # params is :engine_name
    leafs = []
    params[:parent_engine] = params[:engine_name] if !params.key?(:parent_engine)
  ...
  def add_to_managed_engines_registry(service_hash)
   ...
   return log_error_mesg('no_type tree ',service_hash) if !engines_type_tree.is_a?(Tree::TreeNode)
    ...
    return log_error_mesg('no service type node', service_hash) if !service_type_node.is_a?(Tree::TreeNode)
  ...
  def managed_engines_type_registry(site_hash)
    ...
    return log_error_mesg('Site hash missing :container_type', site_hash) if !site_hash.key?(:container_type)
