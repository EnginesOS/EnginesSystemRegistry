class ManagedEnginesRegistry < SubRegistry
  ...
  def add_to_managed_engines_registry(service_hash)
    ...
    p service_hash.to_s
    if service_hash.key?(:parent_engine) == false || service_hash[:parent_engine].nil? == true
